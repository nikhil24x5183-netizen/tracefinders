...
# ----------------- PRINTABLE A4 PDF DOCUMENTATION ENGINE -----------------
@app.get("/api/docs/print/{doc_name}", response_class=HTMLResponse)
def print_documentation_pdf(doc_name: str):
    root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    
    clean_name = doc_name.upper().replace(".MD", "").replace("-", "_")
    valid_docs = {
        "README": "README.md",
        "ARCHITECTURE": "ARCHITECTURE.md",
        "RESEARCH_NOTES": "RESEARCH_NOTES.md",
        "API_SPECIFICATION": "API_SPECIFICATION.md",
        "SECURITY_AND_COMPLIANCE": "SECURITY_AND_COMPLIANCE.md",
        "USER_MANUAL": "USER_MANUAL.md",
        "DOCUMENTATION_INDEX": "DOCUMENTATION_INDEX.md",
        "INDEX": "DOCUMENTATION_INDEX.md"
    }

    filename = valid_docs.get(clean_name, "README.md")
    filepath = os.path.join(root_dir, filename)

    content_md = f"# Document {filename} Not Found"
    if os.path.exists(filepath):
        with open(filepath, "r", encoding="utf-8") as f:
            content_md = f.read()

    # Simple Markdown HTML formatter for A4 PDF rendering
